...
HD inline Coord sph_l2g(const Coord &l, const void *m) {
  ...
  return {r * std::sin(φ) * std::cos(θ), r * std::sin(φ) * std::sin(θ),
          r * std::cos(φ)};
}
HD inline Coord sph_g2l(const Coord &g, const void *m) {
  ...
  return {(r - p->r_min) / (p->r_max - p->r_min), std::atan2(y, x),
          std::acos(z / r)};
}
HD inline bool sph_valid(const Coord &l, const void *) {
  return (l[0] >= 0.0 && l[0] <= 1.0) && (l[1] >= 0.0 && l[1] <= 2 * M_PI) &&
         (l[2] >= 0.0 && l[2] <= M_PI);
}
...
HD inline Coord cube_l2g(const Coord &l, const void *m) {
  ...
  switch (p->face) {
  case Face::PX:
    return {r / d, r * ξ / d, r * η / d};
  case Face::NX:
    return {-r / d, -r * ξ / d, r * η / d};
  case Face::PY:
    return {-r * ξ / d, r / d, r * η / d};
  case Face::NY:
    return {r * ξ / d, -r / d, r * η / d};
  case Face::PZ:
    return {r * ξ / d, r * η / d, r / d};
  case Face::NZ:
    return {r * ξ / d, -r * η / d, -r / d};
  }
  return {0, 0, 0};
...
HD inline bool cube_valid(const Coord &l, const void *) {
  return (l[0] >= -1.0 && l[0] <= 1.0) && (l[1] >= -1.0 && l[1] <= 1.0) &&
         (l[2] >= 0.0 && l[2] <= 1.0);
}
...
public:
  ...
  HD Coord global_to_local(const Coord &g, std::size_t &id_out) const {
    for (std::size_t i = 0; i < count_; ++i) {
      ...
      if (patches_[i].map.is_valid_local &&
          patches_[i].map.is_valid_local(l, patches_[i].meta)) {
        id_out = i;
        return l;
      }
    }
...
inline MultiPatch<7> make_multipatch_cube_sphere(double cube_size, double r_in,
                                                 double r_out) {
